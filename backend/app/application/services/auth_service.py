...
class AuthService:
    ...
    async def login_with_tokens(self, email: str, password: str) -> AuthToken:
        ...
        await self.token_service.register_refresh_token(refresh_token)
    ...
    async def refresh_access_token(self, refresh_token: str) -> AuthToken:
        ...
        # Consume the old refresh token before issuing its replacement. A
        # replay returns None and revokes the family in TokenService.
        consumed = await self.token_service.rotate_refresh_token(refresh_token)
        if not consumed:
        ...
        new_refresh_token = self.token_service.create_refresh_token(
            user, family_id=payload.get("family_id")
        )
        await self.token_service.register_refresh_token(new_refresh_token)
