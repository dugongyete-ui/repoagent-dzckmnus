...
def test_upload_file_success(client, sample_text_file, authenticated_headers):
    """Test successful file upload"""
    url = f"{BASE_URL}/files"
    
    with open(sample_text_file, 'rb') as f:
        files = {'file': ('test_file.txt', f, 'text/plain')}
        response = client.post(url, files=files, headers=authenticated_headers)
...
def test_upload_file_without_file(client):
    ...
    # Authentication is enforced before multipart validation on this protected route.
    assert response.status_code == 401


def test_upload_empty_file(client, authenticated_headers):
...
def test_get_file_info_success(client, sample_text_file, authenticated_headers):
    """Test getting file information"""
    # First upload a file
    upload_url = f"{BASE_URL}/files"
    with open(sample_text_file, 'rb') as f:
        files = {'file': ('info_test.txt', f, 'text/plain')}
        upload_response = client.post(upload_url, files=files, headers=authenticated_headers)
    ...
    response = client.get(info_url, headers=authenticated_headers)
...
def test_get_file_info_not_found(client, authenticated_headers):
...
def test_download_file_success(client, sample_text_file, sample_file_content, authenticated_headers):
    ...
    response = client.get(download_url, headers=authenticated_headers)
...
def test_download_file_not_found(client, authenticated_headers):
...
def test_delete_file_success(client, sample_text_file, authenticated_headers):
    ...
    response = client.delete(delete_url, headers=authenticated_headers)
...
def test_upload_binary_file(client, authenticated_headers):
    ...
    download_url = f"{BASE_URL}/files/{file_id}/download"
    download_response = client.get(download_url, headers=authenticated_headers)
