...
from app.domain.models.event import AgentEvent, ToolEvent
...
class AgentService:
    ...
    async def clear_unread_message_count(self, session_id: str, user_id: str) -> None:
        ...
        session = await self._session_repository.find_by_id_and_user_id(session_id, user_id)
        if not session:
            logger.warning("Refusing unread reset for session %s and user %s", session_id, user_id)
            raise RuntimeError("Session not found")
        await self._session_repository.update_unread_message_count_for_user(session_id, user_id, 0)
    ...
    async def shell_view(self, session_id: str, shell_session_id: str, user_id: str) -> ShellViewResponse:
        ...
        # Only expose shell sessions that this session actually created through
        # a shell tool call. The sandbox process namespace is shared, so the
        # session ownership check alone is not sufficient for an arbitrary ID.
        shell_functions = {
            "shell_exec",
            "shell_view",
            "shell_wait",
        ...
        owns_shell = any(
            ...
            and event.function_name in shell_functions
            and str(event.function_args.get("id", "")) == shell_session_id
            for event in session.events
        )
        if not owns_shell:
            raise RuntimeError("Shell session not found for this session")

        # Get sandbox and shell output through the same user-scoped facade used
        # by the agent tools. Process IDs are additionally bound above to the
        # persisted application session.
        sandbox = await self._sandbox_cls.get(session.sandbox_id)
        if not sandbox:
            raise RuntimeError("Sandbox environment not found")
        scoped_sandbox = UserScopedSandbox(sandbox, session.user_id)
        result = await scoped_sandbox.view_shell(shell_session_id, console=True)
        if result.success:
    ...
    async def file_view(self, session_id: str, file_path: str, user_id: str) -> FileViewResponse:
        ...
        # Get sandbox and file content through the server-side user-scoped
        # facade; prompt instructions are not the authorization boundary.
        sandbox = await self._sandbox_cls.get(session.sandbox_id)
        if not sandbox:
            raise RuntimeError("Sandbox environment not found")
        scoped_sandbox = UserScopedSandbox(sandbox, session.user_id)
        result = await scoped_sandbox.file_read(file_path)
        if result.success:
    ...
    async def get_shared_session_files(self, session_id: str) -> List[FileInfo]:
        ...
        if not getattr(session, "share_files", False):
            logger.warning("Public file access is disabled for shared session %s", session_id)
            return []
        return session.files
    ...
    async def share_session(
        ...
        """Share a session with explicit file policy and optional expiry."""
        ...
        if expires_in_minutes is not None and not 5 <= expires_in_minutes <= 10080:
            raise ValueError("share expiry must be between 5 minutes and 7 days")
        share_expires_at = (
            datetime.now(UTC) + timedelta(minutes=expires_in_minutes)
            if expires_in_minutes is not None else None
        )
        ...
        logger.info(
            "Session %s shared successfully (share_files=%s, expires_at=%s)",
            session_id,
            share_files,
            share_expires_at,
        )
    ...
    async def unshare_session(self, session_id: str, user_id: str) -> None:
        ...
        await self._session_repository.update_shared_status(session_id, False, share_files=False)
    ...
    async def get_shared_session(self, session_id: str) -> Optional[Session]:
        ...
        if not session or not session.is_shared:
            logger.error(f"Shared session {session_id} not found or not shared")
            return None
        expires_at = getattr(session, "share_expires_at", None)
        if expires_at is not None:
            if expires_at.tzinfo is None:
                expires_at = expires_at.replace(tzinfo=UTC)
            if expires_at <= datetime.now(UTC):
                logger.info("Shared session %s has expired", session_id)
                return None
        return session
