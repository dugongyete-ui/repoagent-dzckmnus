...
class MCPDynamicTool:
    """Runtime adapter for one dynamically discovered MCP function."""

    def __init__(self, schema: Dict[str, Any], toolkit: "MCPToolkit") -> None:
        function = schema.get("function", {})
        self.name = str(function.get("name", ""))
        self.description = str(function.get("description", self.name))
    ...
    async def ainvoke(self, input: Any, config: Any = None, **kwargs: Any) -> ToolMessage:
        ...
        if not isinstance(args, dict):
            args = {}
        result = await self.toolkit.invoke_function(self.name, **args)
...
class MCPToolkit(BaseToolkit):
    ...
    def get_tool(self, tool_name: str) -> Optional[MCPDynamicTool]:
        """Return a callable adapter for a dynamically discovered MCP schema."""
        for schema in self._tools:
            function = schema.get("function", {}) if isinstance(schema, dict) else {}
            if function.get("name") == tool_name:
                return MCPDynamicTool(schema, self)
        return None
